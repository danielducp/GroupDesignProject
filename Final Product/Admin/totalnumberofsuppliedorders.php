...
<body style="background-color:#AEB9C7">
  ...
  <h1 align=center> Number of times a supplier has been used </h1>
...
<?php 
    ...
    $stmt = $pdo->prepare("SELECT *, count(suppliedorder.SupplierID) as NumberofOrders FROM suppliedorder INNER JOIN supplier on suppliedorder.SupplierID = supplier.SupplierID GROUP by suppliedorder.SupplierID order by NumberofOrders DESC"); 
    $stmt->execute([]);
    $totalnumberofrows = $stmt ->rowCount();
    $total_pages = ceil($totalnumberofrows / $numberofrecords);
    $stmt = $pdo->prepare("SELECT *, count(suppliedorder.SupplierID) as NumberofOrders FROM suppliedorder INNER JOIN supplier on suppliedorder.SupplierID = supplier.SupplierID GROUP by suppliedorder.SupplierID order by NumberofOrders DESC LIMIT $offset, $numberofrecords "); 
$stmt->execute();
...
{    
    ...
    echo "<TD align=center>Supplier ID: ".$row['SupplierID']."";
    echo "<br><align=center>Supplier Name: ".$row['SupplierName']."";
