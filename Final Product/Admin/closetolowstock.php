...
<h1 align=center>Items Close to Low Stock Level </h1><br>
<?php 
       require ("config.php");
    ...
    $stmt = $pdo->prepare("SELECT ProductName, CurrentStockLevel, LowStockLevel, CurrentStockLevel-LowStockLevel AS Difference  from `product` WHERE CurrentStockLevel-LowStockLevel < 5 "); 
    $stmt->execute([]);
    $totalnumberofrows = $stmt ->rowCount();
    $total_pages = ceil($totalnumberofrows / $numberofrecords);
    $stmt = $pdo->prepare("SELECT ProductName, CurrentStockLevel, LowStockLevel, CurrentStockLevel-LowStockLevel as Difference  from `product`   WHERE CurrentStockLevel-LowStockLevel < 5 LIMIT $offset, $numberofrecords "); 
$stmt->execute();
...
{
    echo "<TD align=center>Order Number: ".$row['ProductName']."";
    echo "<br><align=center>Difference between Low Stock and Current Stock: ".$row['Difference']."";



    echo "<div class=resultbutton onclick=location.href='closetolowstock.php?ProductName=".$row['ProductName']."'>More Information</div></td>";
    echo "</TR>";
  }
  echo "</TABLE>";
?>
