...
<body style="background-color:#AEB9C7">
  ...
  <h1 align=center> Number of times a product has been ordered </h1>
...
<?php 
    ...
    $stmt = $pdo->prepare("SELECT *, count(orderedproducts.ProductCode) as NumberofOrders FROM orderedproducts INNER JOIN `product` on orderedproducts.ProductCode = `product`.ProductCode GROUP by orderedproducts.ProductCode order by NumberofOrders DESC"); 
    $stmt->execute([]);
    $totalnumberofrows = $stmt ->rowCount();
    $total_pages = ceil($totalnumberofrows / $numberofrecords);
    $stmt = $pdo->prepare("SELECT *, count(orderedproducts.ProductCode) as NumberofOrders FROM orderedproducts INNER JOIN `product` on orderedproducts.ProductCode = `product`.ProductCode GROUP by orderedproducts.ProductCode order by NumberofOrders DESC LIMIT $offset, $numberofrecords "); 
$stmt->execute();
...
{    
    ...
    echo "<TD align=center>Supplier ID: ".$row['ProductCode']."";
    echo "<br><align=center>Supplier Name: ".$row['ProductName']."";
    echo "<br><align=center>Total Number of Orders: ".$row['NumberofOrders']."";
